...
# iOS 英文档案 relationship_type → 强制 iOS 分类（小写匹配）
# iOS 存储的 12 种关系标签：Manager / Subordinate / Partner / Best Friend / Friend /
# Teacher / Classmate / Neighbor / Family / Pet / Colleague / Self
# Self 保持 skip（不强制分类）
_KG_REL_TYPE_MAP: dict[str, str] = {
    "boss":         "work_life",
    "manager":      "work_life",
    "subordinate":  "work_life",
    "colleague":    "work_life",
    "family":       "family",
    ...
    "teacher":      "campus_life",
    "classmate":    "campus_life",
    "neighbor":     "life_skills",
    "pet":          "personal_growth",
}
...
def _forced_category_from_profiles(profiles: list[dict]) -> str | None:
    for p in profiles or []:
        ...
        # 1. Chinese map (for Chinese-language users)
        for cat, rel_set in _PROFILE_CATEGORY_MAP.items():
            if rel in rel_set:
                logger.info(f"[档案强制] relationship_type={rel} → category={cat} (ZH map)")
                return cat
        # 2. English map (for English iOS users; case-insensitive)
        cat = _KG_REL_TYPE_MAP.get(rel.lower())
        if cat:
            logger.info(f"[档案强制] relationship_type={rel} → category={cat} (EN map)")
            return cat
    return None
