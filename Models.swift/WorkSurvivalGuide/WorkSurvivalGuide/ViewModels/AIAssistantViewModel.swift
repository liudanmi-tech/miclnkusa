...
struct AssistantMessage: Identifiable, Codable {
    ...
    /// nil = 无图；"loading" = 生成中（骨架屏）；其他 = 真实图片 URL
    let sceneImageURL: String?

    var isMeme: Bool  { memeURL != nil }
    var isVoice: Bool { audioFileURL != nil }
    /// true = 正在生成图片，显示骨架屏
    var isGeneratingSceneImage: Bool { sceneImageURL == "loading" }
    /// true = 图片已就绪
    var hasSceneImage: Bool {
        guard let url = sceneImageURL else { return false }
        return url != "loading"
    }
    ...
    static func voice(fileURL: String) -> AssistantMessage {
        AssistantMessage(id: UUID(), role: .user, content: "[Voice message]", timestamp: Date(),
                         memeURL: nil, skillName: nil, audioFileURL: fileURL, sceneImageURL: nil)
    }

    private init(id: UUID, role: MessageRole, content: String, timestamp: Date,
                 memeURL: String?, skillName: String?, audioFileURL: String?, sceneImageURL: String?) {
        self.id = id; self.role = role; self.content = content
        self.timestamp = timestamp; self.memeURL = memeURL; self.skillName = skillName
        self.audioFileURL = audioFileURL; self.sceneImageURL = sceneImageURL
    }

    /// 返回附带场景图片 URL 的副本（"loading" = 骨架屏；nil = 清除；其他 = 真实 URL）
    func withSceneImage(_ url: String?) -> AssistantMessage {
        AssistantMessage(id: id, role: role, content: content, timestamp: timestamp,
